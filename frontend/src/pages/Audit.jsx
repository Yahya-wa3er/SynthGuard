...
import { analyzeAudit , getHistory } from '../services/api'
...
export default function Audit() {
    ...
    pollTimer.current = setInterval(async () => {
      ...
      try {
        ...
        const found = docs.find(
          d => d.account === accountName && d.module === 'audit'
        )
        if (found) {
  ...
  const handleSubmit = async (data) => {
    ...
    clearInterval(pollTimer.current)
    try {
      const res = await analyzeAudit(data)
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-primary">Audit Automatisé</h1>
        <p className="text-sm text-gray-400 mt-1">Générez un rapport d'audit automatique en quelques secondes</p>
      </div>
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <div>
          {result ? (
            <ResultCard result={result} id={docId} module="audit" />
          ) : (
