...
import { analyzeCompliance , getHistory } from '../services/api'
...
export default function Compliance() {
    ...
    pollTimer.current = setInterval(async () => {
      ...
      try {
        ...
        const found = docs.find(
          d => d.account === accountName && d.module === 'compliance'
        )
        if (found) {
  ...
  const handleSubmit = async (data) => {
    ...
    clearInterval(pollTimer.current)
    try {
      const res = await analyzeCompliance(data)
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-primary">Compliance & AML</h1>
        <p className="text-sm text-gray-400 mt-1">Détectez les risques de blanchiment et fraude fiscale</p>
      </div>
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <div>
          {result ? (
            <ResultCard result={result} id={docId} module="compliance" />
          ) : (
