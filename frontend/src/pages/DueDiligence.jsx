...
import { analyzeDueDiligence, getHistory } from '../services/api'
...
export default function DueDiligence() {
  ...
  // Polling MongoDB via /history si l'API retourne "queued"
  const pollForResult = (accountName) => {
    let attempts = 0
    const MAX = 30 // 30 × 500ms = 15s max
    setWaiting(true)
    ...
    pollTimer.current = setInterval(async () => {
      ...
      try {
        ...
        const found = docs.find(
          d => d.account === accountName && d.module === 'due_diligence'
        )
        if (found) {
          clearInterval(pollTimer.current)
          setWaiting(false)
          // Reconstruire le result depuis le doc historique
          setDocId(found.id)
          setResult({
  ...
  const handleSubmit = async (data) => {
    ...
    try {
      const res = await analyzeDueDiligence(data)

      if (res.data.status === 'ok') {
        setResult(res.data.result)
        setDocId(res.data.id)
      } else {
        // status === 'queued' → on poll l'historique
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-primary">Due Diligence</h1>
        <p className="text-sm text-gray-400 mt-1">
          Analysez un prospect B2B avant signature de contrat
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <div>
          {result ? (
            <ResultCard result={result} id={docId} module="due_diligence" />
          ) : (
